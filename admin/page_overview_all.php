...
while($row=mysqli_fetch_assoc($query_list_country)){
	...
	echo '<i class="fa fa-dot-circle-o" aria-hidden="true"></i> Mã quốc gia:<b>'.$row['country_code'].'</b> <br/> <a class="db" table="lang_'.$key_country.'" href="'.$url_admin.'/?page_view=page_lang&lang='.$key_country.'"><i class="fa fa-dot-circle-o" aria-hidden="true"></i> Từ khóa ngôn ngữ:<b>'.$row['key'].'</b> <span class="syn lang_'.$key_country.'" syn="lang_'.$key_country.'"></span> ('.$data_count_lang_value['c'].')</a></br>';
	echo '<ul>';
	echo '<li><a class="db" table="app_my_girl_user_'.$key_country.'" limit="5000" href="'.$url.'/admin/?page_view=page_login_manager&lang='.$key_country.'"><span class="syn app_my_girl_user_'.$key_country.'" syn="app_my_girl_user_'.$key_country.'"></span> Số tài khoản đã xác thực:<b>'.$data_count_account['c'].'</b></a></li>';
	...
	echo '<li>';
		...
		echo '<ul>';
		echo '<li><span class="db" table="product_name_'.$key_country.'" class="syn product_name_'.$key_country.'" syn="product_name_'.$key_country.'"></span>Tiêu đề (<b>'.$data_count_product_name['c'].'</b>)</li>';
		echo '<li><span class="db" table="product_desc_'.$key_country.'" class="syn product_desc_'.$key_country.'" syn="product_desc_'.$key_country.'"></span>Mô tả (<b>'.$data_count_product_desc['c'].'</b>)</li>';
		echo '</ul>';
	echo '</li>';
	...
	if($sub_view=='full'){
		echo '<li>';
			...
			echo '<ul>';
			echo '<li><a class="db" table="app_my_girl_msg_'.$key_country.'" href="'.$url.'/app_my_girl_msg.php?lang='.$key_country.'&character_sex=1" target="_blank">Msg</a> <span class="syn app_my_girl_msg_'.$key_country.'" syn="app_my_girl_msg_'.$key_country.'"></span>(<b>'.$data_count_msg['c'].'</b>)</li>';
			echo '<li><a class="db" table="app_my_girl_'.$key_country.'" limit="1000" href="'.$url.'/app_my_girl_chat.php?lang='.$key_country.'&character_sex=1" target="_blank">Chat</a> <span class="syn app_my_girl_'.$key_country.'" syn="app_my_girl_'.$key_country.'"></span>(<b>'.$data_count_chat['c'].'</b>)</li>';
			echo '<li><a class="db" table="app_my_girl_field_'.$key_country.'" href="#">Trường dữ liệu Chat</a> <span class="syn app_my_girl_field_'.$key_country.'" syn="app_my_girl_field_'.$key_country.'"></span>(<b>'.$data_count_field_chat['c'].'</b>)</li>';
			echo '<li><a class="db" table="app_my_girl_'.$key_country.'_lyrics" limit="200" href="'.$url.'/app_my_girl_music_lyrics.php?lang='.$key_country.'" target="_blank">Lyrics</a> <span class="syn app_my_girl_'.$key_country.'_lyrics" syn="app_my_girl_'.$key_country.'_lyrics"></span>(<b>'.$data_count_lyrics['c'].'</b>)</li>';
			echo '<li><a class="db" table="app_my_girl_video_'.$key_country.'" href="'.$url.'/app_my_girl_music_link_youtube.php?lang='.$key_country.'" target="_blank">Youtube link</a> <span class="syn app_my_girl_video_'.$key_country.'" syn="app_my_girl_video_'.$key_country.'"></span>(<b>'.$data_count_ytb['c'].'</b>)</li>';
			echo '<li><a class="db" table="app_my_girl_music_data_'.$key_country.'" href="#">Đánh giá âm nhạc</a> <span class="syn app_my_girl_music_data_'.$key_country.'" syn="app_my_girl_music_data_'.$key_country.'"></span>(<b>'.$data_count_rate_music['c'].'</b>)</li>';
			echo '</ul>';
		echo '</li>';
	}
