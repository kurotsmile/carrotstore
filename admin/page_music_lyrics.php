<?php
$list_lyrics=mysql_query("SELECT * FROM `music_contribute_lyrics`"); 

if(isset($_GET['delete'])){
    $id_music=$_GET['delete'];
    $lang=$_GET['lang'];
    $query_delete=mysql_query("DELETE FROM `music_contribute_lyrics` WHERE `id_music` = '$id_music' AND `lang` = '$lang' LIMIT 1;");
...
<?php
while($row=mysql_fetch_array($list_lyrics)){
?>
    <tr>
        ...
        <td>
            ...
            <a class="buttonPro small red" href="<?php echo $url_admin;?>/?page_view=page_music_lyrics&delete=<?php echo $row['id_music'];?>"><i class="fa fa-trash" aria-hidden="true"></i> Xóa</a>
        </td>
    </tr>
<?php
}
mysql_free_result($list_lyrics);
?>
</table>