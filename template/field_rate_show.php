...
<div style="height: 28px;margin: 2px;display: inline-block;">
    <?php
    $count_rate=mysqli_query($link,"SELECT `rate` FROM `".$type_rate."_rate` WHERE `".$type_rate."` = '".$data['id']."'");
    if($count_rate){
        $count_rate=mysqli_num_rows($count_rate);
        $sum_rate=mysqli_query($link,"SELECT SUM(`rate`) FROM `".$type_rate."_rate` WHERE `".$type_rate."` = '".$data['id']."'");
