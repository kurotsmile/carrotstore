<h1 ><?php echo lang($link,'dk_tai_khoan');?> <i style="float: right;cursor: pointer" onclick="$('#box_regiter_member').fadeOut(100).remove();" class="fa fa-times-circle"></i></h1>
<form id="form_register">

        <p>
            <label for="reg_username"><?php echo lang($link,'ten_dang_nhap');?></label><br />
            <input class="inp" type="text" name="reg_username" id="reg_username" value="" />
        </p>

        <p>
            <label for="reg_password"><?php echo lang($link,'mat_khau');?></label><br />
        ...
        <p id="reg_rep_password_are" style="display: none;">
            <label for="reg_rep_password"><?php echo lang($link,'nhap_lai_mat_khau');?></label><br />
        ...
        <p>
            <label for="reg_address"><?php echo lang($link,'dia_chi');?></label><br />
        ...
        <p>
            <label for="reg_contry"><?php echo lang($link,'quoc_gia');?></label><br />
        ...
        <p><button class="buttonPro purple" onclick="register_account(this);return false;"><?php echo lang($link,'dang_ky'); ?></button></p>
    </form>
    <script>
        function register_account(emp){
            ...
            if(txt==null || txt==''){
                $('#reg_username').after('<p class="error"><?php echo lang($link,'ten_dn_k_de_trong'); ?></p>');
            ...
            if(txt==null || txt==''){
                $('#reg_password').after('<p class="error"><?php echo lang($link,'mk_k_dc_de_trong'); ?></p>');
                swal("<?php echo lang($link,'loi'); ?>","<?php echo lang($link,'mk_k_dc_de_trong'); ?>", "error");
                return false;
            }
            ...
            if(txt!=txt2){
                $('#reg_rep_password').after('<p class="error"><?php echo lang($link,'mk_nhap_lai_sai'); ?></p>');
                swal("<?php echo lang($link,'loi'); ?>","<?php echo lang($link,'mk_nhap_lai_sai'); ?>", "error");
                return false;
            }


            var txt_data=$("#form_register").serialize();
            $(emp).after('<p class="alert"><?php echo lang($link,'dang_xu_ly'); ?></p>');
            $('#loading').fadeIn(200);
            $.ajax({
                url: "index.php",
                type: "get", //ki�?u du? li�?u truy�?n ?i
                data: "function=box_register&"+txt_data, //tham s�? truy�?n va?o
                success: function(data, textStatus, jqXHR)
                {
                    $('.alert').remove();
                    if(data=='1'){
                        swal("<?php echo lang($link,'loi'); ?>","<?php echo lang($link,'loi_tk_ton_tai'); ?>", "error");
                    }else{
                        swal("<?php echo lang($link,'thanh_cong'); ?>","<?php echo lang($link,'dk_thanh_cong'); ?>", "success");
        ...
        function update_box_account(){
            ...
            $.ajax({
                url: "index.php",
                type: "get", //ki�?u du? li�?u truy�?n ?i
                data: "function=box_register_update&reg_sdt="+txt_sdt+"&reg_avt="+txt_icon+"&reg_user="+txt_username+"&full_name_reg="+txt_full_name, //tham s�? truy�?n va?o
                success: function(data, textStatus, jqXHR)
                {
                    swal("<?php echo lang($link,'cap_nhat'); ?>",data, "success");
                    $('#loading').fadeOut(200);
