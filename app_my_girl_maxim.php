...
$txt_query_effect="AND  `effect`='36'";
...
if(isset($_POST['loc'])){
    ...
    if($tip!=''){
        $result_tip=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE  `tip` = '$tip'  $txt_key_search $txt_query_effect $limit_chat_query $disable_chat_sql ORDER BY `id` DESC $limit_txt_query");
    }else{
        $result_tip=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE  `disable` = 0  $txt_key_search $txt_query_effect $limit_chat_query $disable_chat_sql ORDER BY `id` DESC $limit_txt_query");
    }
}else{
    $result_tip=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = 0  $txt_query_effect ORDER BY `id` DESC  $limit_txt_query");
}

$result_tip_all=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = 0  $txt_query_effect ORDER BY `id`");
?>
...
<div id="form_loc" style="font-size: 11px;">
    <div style="display: inline-block;float: left;margin: 2px;">
        <i class="fa fa-music" aria-hidden="true"></i> có <?php echo mysql_num_rows($result_tip);?> hiển thị / <?php echo mysql_num_rows($result_tip_all);?> châm ngôn
    </div>
</div>
...
echo '<tr style="border:solid 1px green"><th>id</th><th>Loại</th><th>Trả lời</th><th>chat</th><th>Giới tính</th><th>Chức năng</th><th>Giới hạng</th><th>Giọng</th><th>Action</th><th>Action</th></tr>';
        while ($row = mysql_fetch_array($result_tip)) {
            ...
            echo show_row_chat_prefab($row,$langsel,' '.$bnt_del.' '.$check_select.' '.$bnt_view_lyric);
        }
echo '</table>';
