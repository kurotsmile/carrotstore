...
if($function=='save_playlist_music'){
    ...
    if(mysqli_num_rows($query_playlist)==0){
        ...
        while($item_playlist=mysqli_fetch_array($query_playlist)){
            ...
            $txt_show.='<td>';
            $txt_show.='<span class="buttonPro small blue" onclick="add_song_to_playlist(\''.$item_playlist['id'].'\',\''.$id_music.'\',\''.$lang.'\',\''.$lang_music.'\')"><i class="fa fa-plus-circle" aria-hidden="true"></i> '.lang($link,'add_song_to_playlist',$lang).'</span>';
            $txt_show.='<span class="buttonPro small red" onclick="delete_playlist_music(\''.$item_playlist['id'].'\',\''.$lang.'\')"><i class="fa fa-trash" aria-hidden="true"></i> '.lang($link,'delete',$lang).'</span>';
            $txt_show.='</td>';
            $txt_show.='</tr>';
        }
        $txt_show.='</table>';
        $txt_show.='<span class="buttonPro" onclick="create_playlist(\''.$id_music.'\',\''.$lang.'\',\''.$id_user.'\')"><i class="fa fa-plus-square" aria-hidden="true"></i> '.lang($link,'create_playlist',$lang).'</span>';
        $txt_show.='<span class="buttonPro" onclick="swal.close();"><i class="fa fa-times-circle" aria-hidden="true"></i> '.lang($link,'back',$lang).'</span>';
        $alert->msg=$txt_show;
    }
