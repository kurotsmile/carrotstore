...
if($function=='list_country'){
    ...
    $query_country=mysqli_query($link,"SELECT a.`id`,a.`name`,a.`key` FROM carrotsy_virtuallover.`app_my_girl_country` as a INNER JOIN carrotsy_bible.`country` as b ON b.key = a.key");
    $arr_data=array();
    while($item_country=mysqli_fetch_assoc($query_country)){
...
if($function=='download_lang'){
    ...
    $query_value_lang=mysqli_query($link,"SELECT `value` FROM `lang_value` WHERE `id_country` = '$key' LIMIT 1");
    $data_val=mysqli_fetch_assoc($query_value_lang);
