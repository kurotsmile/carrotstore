...
if($id_edit!=''){
    $query_book=mysqli_query($this->link_mysql,"SELECT * FROM `book` WHERE `id` = '$id_edit' LIMIT 1");
...
$query_count_book=mysqli_query($this->link_mysql,"SELECT * FROM `book` WHERE `lang` = '$lang_book' AND `type`=$type_book ");
...
<form class="box_form" action="" method="post">
    ...
    if(isset($_POST['name_book'])){
        ...
        if($id_edit==''){
            $query_add=mysqli_query($this->link_mysql,"INSERT INTO `book` (`name`, `lang`,`type`,`chapter`,`orders`) VALUES ('$name_book', '$lang_book',$type_book,$chapter_book,$orders);");
            if($query_add){
                $name_book='';
    ...
    <div class="row">
        ...
        <a class="buttonPro small blue" href="<?php echo $this->url; ?>/?page=view_book&id=<?php echo $id_edit;?>">Trở về quả lý sách</a>
        <?php }else{?>
...
<ul style="float: left;width: 100%;">
    <li>
    <a class="buttonPro small blue" href="<?php echo $this->url;?>/?page=manager_book&lang=<?php echo $lang_book;?>&type=<?php echo $type_book;?>">Quản lý sách đã thêm</a>
    </li>
</ul>
