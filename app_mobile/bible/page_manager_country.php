...
if(isset($_POST['sel_country'])){
    ...
    $query_reste_country=mysqli_query($this->link_mysql,"DELETE FROM `country`");
    for($i=0;$i<sizeof($sel_country);$i++){
        $query_add=mysqli_query($this->link_mysql,"INSERT INTO `country` (`key`) VALUES ('".$sel_country[$i]."');");
    }
...
?>
    <tr>
        <td>
            <img src="<?php echo $this->url_carrot_store;?>/thumb.php?src=<?php echo $this->url_carrot_store;?>/app_mygirl/img/<?php echo $key_country;?>.png&size=30&trim=1" />
        </td>
        <td>
