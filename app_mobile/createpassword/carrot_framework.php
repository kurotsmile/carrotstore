...
if($function=='list_country'){
    ...
    $query_country=mysqli_query($link,"SELECT vl.`id`,vl.`name`,vl.`key` FROM carrotsy_virtuallover.app_my_girl_country as vl , carrotsy_createpassword.country as p WHERE p.`key`=vl.`key`");
    $arr_data=array();
    while($item_country=mysqli_fetch_assoc($query_country)){
...
if($function=='download_lang'){
    ...
    $query_data_lang_app=mysqli_query($link,"SELECT `value` FROM  carrotsy_createpassword.`value_lang_app` WHERE `id_country` = '$key' LIMIT 1");
    $data_lang_app=mysqli_fetch_assoc($query_data_lang_app);
    $data_lang->{"lang_app"}=$data_lang_app['value'];
