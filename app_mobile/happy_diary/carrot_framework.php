...
if($function=='list_country'){
    ...
    $query_country=mysqli_query($link,"SELECT vl.`id`,vl.`name`,vl.`key` FROM carrotsy_virtuallover.app_my_girl_country as vl , $mysql_database.country as p WHERE p.`key`=vl.`key`");
    $arr_data=array();
    while($item_country=mysqli_fetch_assoc($query_country)){
...
if($function=='download_lang'){
    ...
    $query_data_lang_app=mysqli_query($link,"SELECT `data` FROM  $mysql_database.`lang_val` WHERE `lang` = '$key' LIMIT 1");
