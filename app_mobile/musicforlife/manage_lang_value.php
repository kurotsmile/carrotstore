...
if(isset($_POST['key'])){
    ...
    if(mysql_num_rows(mysqli_query($link,"SELECT `lang` FROM carrotsy_music.`key_value` WHERE `lang`='$key' "))){
        ...
        if($query_update_data===true){
            ...
            echo btn_add_work($edit_lang,$edit_lang,'app_music','edit');
        }else{
            echo 'Cập nhật dữ liệu giao diện thất bại!';
        }
    }else{
        $query_add_data=mysqli_query($link,"INSERT INTO carrotsy_music.`key_value` (`lang`, `data`) VALUES ('$key', '$data');");
        if($query_add_data===true){
            echo 'Thêm dữ liệu giao diện thành công!';
            echo btn_add_work($edit_lang,$edit_lang,'app_music','add');
        }else{
...
<table>
    <?php
    $data_display='';
    $query_data_display=mysqli_query($link,"SELECT `data` FROM carrotsy_music.`key_value` WHERE `lang` = '$edit_lang' LIMIT 1");
...
function change_lang_to(emp){
    ...
    window.location="<?php echo $url;?>/index.php?view=lang_value&lang=<?php echo $edit_lang;?>&lang_to="+val_lang;
}
