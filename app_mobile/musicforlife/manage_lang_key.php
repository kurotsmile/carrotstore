...
if(isset($_GET['delete'])){
    ...
    $delete_ver=$_GET['ver'];
    $query_delete=mysqli_query($link,"DELETE FROM carrotsy_music.`key_lang` WHERE `key` = '$delete_key' LIMIT 1;");
...
if(isset($_POST['key'])){
    ...
    if(isset($_POST['edit'])){
        $edit=$_POST['edit'];
        $update_data_dislay=mysqli_query($link,"UPDATE carrotsy_music.`key_lang` SET `key` = '$key' WHERE `key` = '$edit'  LIMIT 1;");
        ...
        if(mysql_num_rows(mysqli_query($link,"SELECT * FROM carrotsy_music.`key_lang` WHERE `key` = '$key'  LIMIT 1"))){
            echo '<b>Lỗi:</b> Từ khóa dữ liệu hiển "'.$key.'" thị đã có với phiên bản "'.$ver.'" này!';
        }else{
            $add_data_dislay=mysqli_query($link,"INSERT INTO carrotsy_music.`key_lang` (`key`) VALUES ('$key');");
...
<form id="form_loc"  method="post" enctype="multipart/form-data" name="add_key_lang">
    ...
    <div style="display: inline-block;float: left;margin: 2px;">
        ...
        <?php }else{?>
            ...
            <a class="buttonPro blue" href="<?php echo $url;?>/index.php?view=lang_key">Hủy</a>
        <?php }?>
    </div>
</form>
...
<?php
$query_list_display_lang_data=mysqli_query($link,"SELECT * FROM carrotsy_music.`key_lang`");
while($row=mysqli_fetch_array($query_list_display_lang_data)){
?>
    <tr>
        <td><i class="fa fa-tag" aria-hidden="true"></i> <?php echo $row['key'];?></td>
        <td>
            <a class="buttonPro small orange" href="<?php echo $url;?>/index.php?view=lang_key&edit=<?php echo $row['key'];?>"><i class="fa fa-edit"></i> Sửa giá trị</a>
            <a class="buttonPro small red" onclick="return confirm('Có chắc chắn là muốn xóa?');" href="<?php echo $url.'/app_my_girl_display_lang.php';?>?delete=<?php echo $row['key'];?>"><i class="fa fa-trash" aria-hidden="true"></i> Xóa</a>
        </td>
    </tr>
<?php
}
