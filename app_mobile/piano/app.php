...
if($function=='upload_midi'){
    ...
    $query_add=mysqli_query($link,"INSERT INTO `midi` (`id_midi`,`id_device`, `name`, `data0`, `data1`, `data2`, `type0`, `type1`, `type2`, `speed`, `sell`) VALUES ('$id_midi','$user_id', '$name_midi', '$line0', '$line1', '$line2', '$type0', '$type1', '$type2', '$speed', '0');");
...
if($function=='list_midi_online'){
    ...
    $arr_data=array();
    if($user_id!=''){
        $query_list=mysqli_query($link,"SELECT * FROM `midi` WHERE `id_device`='$user_id' OR `id_device`='$id_device' ORDER BY RAND() LIMIT 20");
    }else{
