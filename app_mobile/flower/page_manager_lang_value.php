...
$url_cur=$url.'/?view=manager_lang_value';
...
if(isset($_POST['lang_key'])){
    ...
    if(mysql_num_rows($check_value_lang)==0){
        ...
        if($query_add_lang){
            ...
            echo btn_add_work('lang_value',$lang_key,'flower','add');
        }else{
        ...
        if($query_update_lang){
            ...
            echo btn_add_work('lang_value',$lang_key,'flower','edit');
        }else{
...
<tr>
    ...
    <td>
        <a href="<?php echo $url;?>?view=manager_lang_key&edit_id=<?php echo $row_key['id']; ?>&name_key=<?php echo $row_key['name_key']; ?>" class="buttonPro small"><i class="fas fa-edit"></i> sửa tag</a>
        <?php 
        if($lang_key_to!=''){
...
function change_lang_to(emp){
    ...
    window.location="<?php echo $url;?>?view=manager_lang_value&lang=<?php echo $lang_key;?>&lang_to="+val_lang;
}
