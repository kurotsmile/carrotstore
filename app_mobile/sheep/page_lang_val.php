...
?>
    <a href="<?php echo $url_cur;?>&lang=<?php echo $country['key'];?>" <?php if($lang==$country['key']){ ?>style="color:blue"<?php }?>><img style="float: left" src="<?php echo $url_carrot_store;?>/thumb.php?src=<?php echo $url_carrot_store;?>/app_mygirl/img/<?php echo $country['key'];?>.png&size=16x16&trim=1"><?php echo $country['name'];?></a>
<?php } ?>
</div>
...
if(isset($_POST['cr_val'])){
    ...
    $query_cr_val=mysqli_query($link,"SELECT `value` FROM `$table_lang_Val` WHERE `id_country` = '$lang' LIMIT 1");
    $data_cr=mysqli_fetch_assoc($query_cr_val);
    if($data_cr==null){
        $query_add_val=mysqli_query($link,"INSERT INTO `$table_lang_Val` (`id_country`, `value`) VALUES ('$lang', '$val_data');");
        echo msg("Thêm mới dữ liệu thành công!");
    }else{
        $query_add_val=mysqli_query($link,"UPDATE `$table_lang_Val` SET `value` = '$val_data' WHERE `id_country` = '$lang' LIMIT 1;");
        echo msg("Cập nhật dữ liệu thành công!");
    }
}

$query_cr_key=mysqli_query($link,"SELECT `key` FROM `$table_lang_key`");
$query_cr_val=mysqli_query($link,"SELECT `value` FROM `$table_lang_Val` WHERE `id_country` = '$lang'");
$data_cr_val=mysqli_fetch_assoc($query_cr_val);
$data_cr=(array)json_decode($data_cr_val['value']);
?>
