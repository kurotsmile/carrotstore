...
if(isset($_GET['lang'])){
    $lang_key=$_GET['lang'];
    $query_get_val=mysqli_query($link,"SELECT `value` FROM `value_lang` WHERE `id_country` = '$lang_key' LIMIT 1");
    $data_val=mysqli_fetch_assoc($query_get_val);
...
if(isset($_POST['lang_key'])){
    ...
    if(mysql_num_rows($check_value_lang)==0){
        ...
        if($query_add_lang){
            ...
            echo alert("Thêm mới các giá trị ngôn ngữ thất bại!!! ".mysqli_error($link),"error");
        }
        ...
        if($query_update_lang){
            ...
            echo alert("Cập nhật các giá trị ngôn ngữ thất bại!!!".mysqli_error($link),"error");
        }
        $data_val=json_decode($data_val);
    }
}
