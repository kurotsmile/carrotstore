...
if($func=="download_lang"){
    $id=$_POST['id'];
    echo json_encode($app_sheep->list_lang[$id]);
    exit;
}
...
if($func=="list_music"){
    ...
    while ($row_music = mysql_fetch_array($query_list_music)) {
        ...
        array_push($app_sheep->list_music,$item_m);
    }
    unset($app_sheep->list_lang);
    echo json_encode($app_sheep);
    exit;
}
...
if($func=="get_good_night"){
    ...
    if($data_good_night["user_type"]=='0'){
        ...
        $good_night->avatar_user="http://carrotstore.com/img.php?url=app_mygirl/app_my_girl_".$lang."_user/$id_good_night_user.png&size=50";
...
if($func=='get_user'){
    ...
    if(mysql_num_rows($query_get_user)>0){
        ...
        $account->avatar="http://carrotstore.com/img.php?url=app_mygirl/app_my_girl_".$lang."_user/$id_device.png&size=80";
        $account->phone=$arr_user['sdt'];
        $account->sex=$arr_user['sex'];
        
        $item_info=new Item_info();
        $item_info->icon='http://sheep.carrotstore.com/image/address.png';
        ...
        $item_info->label='sex';
        if($arr_user['sex']=='0'){
            $item_info->icon='http://sheep.carrotstore.com/image/sex_boy.png';
            $item_info->value='sex_boy';
        }else{
            $item_info->icon='http://sheep.carrotstore.com/image/sex_girl.png';
        ...
        $item_info=new Item_info();
        $item_info->icon='http://sheep.carrotstore.com/image/phone_number.png';
        $item_info->value=$arr_user['sdt'];
        $item_info->label='phone_number';
        array_push($account->arr_item_info,$item_info);
        
        $item_info=new Item_info();
        $item_info->icon='http://sheep.carrotstore.com/image/account_status.png';
        ...
        if(mysql_num_rows($query_app_sheep)>0){
            ...
            $item_info=new Item_info();
            $item_info->icon='http://contact.carrotstore.com/image/sheep.png';
        ...
        if(mysql_num_rows($query_check_field)>0){
            ...
            foreach($data_field as $i_key=>$i_val){
                ...
                if(mysql_num_rows($query_get_field)>0){
                    ...
                    $item_info=new Item_info();
                    $item_info->icon='http://contact.carrotstore.com/field_icon/'.$data_item_field['id'].'.png';
...
if($func=='edit_and_create_user'){
    ...
    if($error==0){
        ...
        if(mysql_num_rows($query_get_user)>0){
            ...
            $query_add_user=mysql_query("INSERT INTO carrotsy_virtuallover.app_my_girl_user_$lang (`id_device`, `name`,`sdt`,`status`,`sex`,`date_start`,`date_cur`) VALUES ('$id_device', '$user_name', '$phone', '0','$sex',NOW(),NOW());");
...
if($func=='list_top_player'){
    ...
    while ($row_player = mysql_fetch_array($query_get_list_top_player)) {
        ...
        if(mysql_num_rows($query_get_user)){
            ...
            $item_player->avatar_user="http://carrotstore.com/img.php?url=app_mygirl/app_my_girl_".$lang."_user/$id_user.png&size=50";
            $index++;
...
if($func=='get_list_app'){
    ...
    while ($app_i = mysql_fetch_array($query_get_app)) {
        ...
        array_push($app_sheep->list_music,$app_item);
    }
    echo json_encode($app_sheep);
    exit;
}
