...
if($func=='add'||$func=='edit'){
    ...
    if(isset($_POST['ads_app_id'])){
        ...
        if($func=='add'){
            $q_add=$this->q("INSERT INTO carrotsy_virtuallover.`app_ads` (`id_app`, `google_Play`, `samsung_galaxy_store`, `microsoft_store`, `amazon_app_store`, `carrot_store`) VALUES ('$ads_app_id', '$ads_Google_Play', '$ads_Samsung_Galaxy_Store', '$ads_Microsoft_Store', '$ads_Amazon_app_store', '$ads_Carrot_store');");
            if($q_add) echo $this->msg("Thêm quảng cáo thành công!");
        }else{
            $q_edit=$this->q("UPDATE carrotsy_virtuallover.`app_ads` SET `google_Play`='$ads_Google_Play',`samsung_galaxy_store`='$ads_Samsung_Galaxy_Store',`microsoft_store`='$ads_Microsoft_Store',`amazon_app_store`='$ads_Amazon_app_store', `carrot_store`='$ads_Carrot_store' WHERE `id_app` = '$ads_app_id' LIMIT 1;");
