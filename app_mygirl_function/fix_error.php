...
<?php
    ...
    function fix_error_by_lang($langsel){
        $count_error=0;
        echo '<strong><i class="fa fa-wrench" aria-hidden="true"></i> Sửa lỗi ('.$langsel.')</strong><br/>';
        
        $query_error_msg=mysql_query("SELECT * FROM `app_my_girl_msg_$langsel` WHERE (`q1` != '' AND `r1` = '') or (`q2` != '' AND `r2` = '') or (`q1`= '' AND `r1` != '') or (`q2`= '' AND `r2` != '')");
        if(mysql_num_rows($query_error_msg)>0){
            echo '<i>Lỗi câu thoại thiếu tham số câu hỏi và câu trả lời</i>';
            echo '<table>';   
        }
        while($row_error_msg=mysql_fetch_array($query_error_msg)){
            $count_error++;
            echo show_row_chat_prefab($row_error_msg,$langsel,'');
        }
        
        if(mysql_num_rows($query_error_msg)>0){
            echo '</table>';   
        }
        mysql_free_result($query_error_msg);
        echo '<hr/>';
        
        
        $query_error=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE (`q1` != '' AND `r1` = '') or (`q2` != '' AND `r2` = '') or (`q1`= '' AND `r1` != '') or (`q2`= '' AND `r2` != '')");
        if(mysql_num_rows($query_error)>0){
            echo '<i>Lỗi trò chuyện thiếu tham số câu hỏi và câu trả lời</i>';
            echo '<table>';   
        }
        while($row_error=mysql_fetch_array($query_error)){
            $count_error++;
        ...
        mysql_free_result($query_error);
        echo '<hr/>';
        
        
        $query_error_no_father=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = '0' AND `id_redirect` != ''");
        $txt_show='';
        if(mysql_num_rows($query_error_no_father)>0){
            while($row_error_no_father=mysql_fetch_array($query_error_no_father)){
                $id_father=$row_error_no_father['id_redirect'];
                $check_redirect=mysql_query("SELECT * FROM `app_my_girl_vi` WHERE `id` = '$id_father' LIMIT 1");
                if(mysql_num_rows($check_redirect)==0){
                    $count_error++;
                    $txt_show.=show_row_chat_prefab($row_error_no_father,$langsel,'');
                }
            }
        }
        mysql_free_result($query_error_no_father);
        ...
        $key_music=get_key_lang('key_music',$langsel);
        $query_error_music_key=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = '0' AND ((`effect` = '2' AND `text` != '$key_music') or (`effect` != '2' AND  `text` = '$key_music'))");
        if(mysql_num_rows($query_error_music_key)>0){
        ...
        mysql_free_result($query_error_music_key);
        
        //kiem tra am thanh
        $key_quote=get_key_lang('key_quote',$langsel);
        $query_error_quote_key=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = '0' AND ((`effect` = '36' AND `text` != '$key_quote') or (`effect` != '36' AND  `text` = '$key_quote')) AND `id_redirect`=''");
        if(mysql_num_rows($query_error_quote_key)>0){
        ...
        mysql_free_result($query_error_quote_key);
        ...
        $query_error_no_sound=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = '0' AND `id_redirect` = '' AND $query_sex ");
        $txt_show='';
        if(mysql_num_rows($query_error_no_sound)>0){
        ...
        mysql_free_result($query_error_no_sound);
        

        $query_error_no_sound_msg=mysql_query("SELECT * FROM `app_my_girl_msg_$langsel` WHERE `disable` = '0' AND $query_sex AND `func`!='thong_bao'");
        $txt_show='';
        if(mysql_num_rows($query_error_no_sound_msg)>0){
        ...
        mysql_free_result($query_error_no_sound_msg);
        ...
        $query_error_no_sound=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = '0' AND `id_redirect` = '' AND $query_sex  AND CHAR_LENGTH(`chat`)>196");
        $txt_show='';
        if(mysql_num_rows($query_error_no_sound)>0){
            while($row_error_no_sound=mysql_fetch_array($query_error_no_sound)){
                $id_row=$row_error_no_sound[0];
        ...
        $query_error_no_sound_msg=mysql_query("SELECT * FROM `app_my_girl_msg_$langsel` WHERE `disable` = '0' AND $query_sex AND CHAR_LENGTH(`chat`)>196 AND `func`!='thong_bao'");
        $txt_show='';
        if(mysql_num_rows($query_error_no_sound_msg)>0){
            while($row_error_no_sound=mysql_fetch_array($query_error_no_sound_msg)){
                $id_row=$row_error_no_sound[0];
        ...
        $txt_show='';
        $query_chat_error_father=mysql_query("SELECT * FROM `app_my_girl_$langsel` WHERE `disable` = '0' AND `pater` = `id`");
