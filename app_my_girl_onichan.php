...
if($func=='list_background'){
    ...
    $list_category=mysqli_query($link,"SELECT * FROM `app_my_girl_bk_category` ORDER BY RAND() LIMIT 10");
...
if($func=='list_radio'){
    ...
    if($id_sub_menu=='1'){
        if($search==''){
            ...
            $list_view=mysqli_query($link,"SELECT * FROM `app_my_girl_radio` WHERE `name_radio` LIKE '%$search%' ORDER BY RAND() LIMIT 21");
        }
    }else{
        if($search==''){
            $list_view=mysqli_query($link,"SELECT * FROM `app_my_girl_radio` WHERE `lang` = '$lang_sel' ORDER BY RAND() LIMIT 21");
        }else{
            $list_view=mysqli_query($link,"SELECT * FROM `app_my_girl_radio` WHERE `lang` = '$lang_sel' AND `name_radio` LIKE '%$search%' ORDER BY RAND() LIMIT 21");
        }
    }
    while($row=mysqli_fetch_array($list_view)){
        $chat=new Chat();
...
if($func=='chat'){
    ...
    if ($result == null) {
        ...
        if($id_question!=''){
                ...
                $get_child_chat=mysqli_query($link,"SELECT * FROM  `$txt_table_chat_return`  WHERE `text`='$text' AND `pater` = '$id_question' AND `pater_type` = '$type_question' AND `sex` = '$sex' AND `character_sex`='$character_sex' AND `disable` = '0' LIMIT 1");
				if($get_child_chat){
