...
<div id="containt" style="width: 100%;float: left;">
    ...
    <?php
    $list_style='same';
    $list_music = mysqli_query($link,"SELECT m.`id`, m.`chat`, m.`file_url`, m.`slug`,m.`author` From `app_my_girl_".$lang_album."` as `m` LEFT JOIN `app_my_girl_".$lang_album."_lyrics` as `l` ON m.id= l.id_music  WHERE l.album  LIKE '%".$name_album."%' ORDER BY RAND() LIMIT 30");
