...
if($func=='check_pay'){
    ...
    $query_check_pay=mysql_query("SELECT * FROM `pay` WHERE `id_user` = '$id_device' AND `id_item` = '$pay_item' AND `status` = '1' LIMIT 1");
    echo mysql_num_rows($query_check_pay);
    exit;
}

if($func=='restore_pay'){
    $query_check_pay_nude=mysql_query("SELECT * FROM `pay` WHERE `id_user` = '$id_device' AND `id_item` = 'obj_nude' AND `status` = '1' LIMIT 1");
    $query_check_pay_removerads=mysql_query("SELECT * FROM `pay` WHERE `id_user` = '$id_device' AND `id_item` = 'obj_nude' AND `status` = '1' LIMIT 1");
    $app->{"obj_nude"}=mysql_num_rows($query_check_pay_nude);
    $app->{"remove_ads"}=mysql_num_rows($query_check_pay_removerads);
...
if($func=='list_ads'){
    $app->all_chat=array();
    $query_list_ads=mysql_query("SELECT * FROM `app_my_girl_ads` WHERE `window` != '' ORDER BY RAND() LIMIT 10");
    while($row_ads=mysql_fetch_array($query_list_ads)){
        ...
        array_push($app->all_chat,$item);
    }
    mysql_free_result($query_list_ads);
}
...
if($func=='chat'){
    ...
    if ($result == null) {
        ...
        if($id_question!=''){
                ...
                $get_child_chat=mysql_query("SELECT * FROM  `$txt_table_chat_return`  WHERE `text`='$text' AND `pater` = '$id_question' AND `pater_type` = '$type_question' AND `disable` = '0' $txt_limit_ver  $txt_limit_os $txt_limit_chat ORDER BY RAND() LIMIT 1");
                ...
                $get_child_chat2=mysql_query("SELECT * FROM  `$txt_table_chat_return` WHERE MATCH (text)  AGAINST ('$text' IN BOOLEAN MODE) AND `pater` = '$id_question' AND `pater_type` = '$type_question' AND `disable` = '0' $txt_limit_ver  $txt_limit_os $txt_limit_chat  LIMIT 1");
...
function chat_func($func){
    ...
    if($version!=''&$os!=''){
        $txt_limit=" AND `ver`!= '$version' AND `os_window`!='1'";
    }
