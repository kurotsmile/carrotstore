...
function get_key_lang($link,$key,$lang){
    ...
    if(mysqli_num_rows($query_get_value)>0){
        $data_val=mysqli_fetch_assoc($query_get_value);
        $val=$data_val['value'];
    }else{
        $val=$key;
    }
...
if($func=='load_ads'){
	$list_ads=mysqli_query($link,"SELECT `id`,`name`,`$os` FROM `app_my_girl_ads` WHERE `$os`!='' ORDER BY RAND()");
	$app->all_chat=array();
    while($row=mysqli_fetch_assoc($list_ads)){
        $item_ads=new chat();
        $item_ads->chat=$row['name'];
        $item_ads->data_text=$row[$os];
        $item_ads->link=thumb('app_mygirl/obj_ads/icon_'.$row['id'].'.png','200');
        array_push($app->all_chat,$item_ads);
    }
