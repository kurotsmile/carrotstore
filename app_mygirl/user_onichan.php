...
if($func=='get_info'){
    ...
    if($s_type=="0"){
        ...
        $id_device_show=$_POST['text'];
        $list_contact = mysqli_query($link,"SELECT * FROM `accounts` WHERE `usernames`='$id_device_show' ORDER BY RAND() LIMIT 1",$link);
        while($row=mysqli_fetch_array($list_contact)){
            $u->id=$row[0];
...
if($func=='get_list_contact'){
    ...
    if($seach!=""){
        $list_users=mysqli_query($link,"SELECT * FROM `app_my_girl_user_$lang_sel` WHERE `status` = '0'  AND `sdt`!='' AND  (MATCH (name)  AGAINST ('$seach' IN BOOLEAN MODE) or `sdt` LIKE '%$seach%') $txt_query_sex ORDER BY RAND() LIMIT 20");
    }else{

        $list_users=mysqli_query($link,"SELECT * FROM `app_my_girl_user_$lang_sel` WHERE `status` = '0'  AND `sdt`!='' $txt_query_sex ORDER BY RAND() LIMIT 21");
    }
    ...
    $arr_data_item=Array('2','5','msg_box_func',get_key_lang($link,'contact_all',$lang_sel),$color_sel);
    array_push($app->all_tip,$arr_data_item);
    
    $color_sel='000000';
    if($id_sub_menu=='0'){$color_sel='ff6347';}
    $arr_data_item=Array('0','5','msg_box_func',get_key_lang($link,'contact_sex_0',$lang_sel),$color_sel);
    array_push($app->all_tip,$arr_data_item);
    
    $color_sel='000000';
    if($id_sub_menu=='1'){$color_sel='ff6347';}
    $arr_data_item=Array('1','5','msg_box_func',get_key_lang($link,'contact_sex_1',$lang_sel),$color_sel);
    array_push($app->all_tip,$arr_data_item);
    echo json_encode($app);
