    ...
    if(isset($_POST['func'])){
        ...
        if($_POST['func']=="add"){
            ...
            while($l=mysqli_fetch_array($list_country)){
                ...
                $add_key_lang=mysqli_query($link,"INSERT INTO `app_my_girl_key_lang` (`key`,`value`,`lang`) VALUES ('$key','$value','$key_lang');");
            ...
            while($l=mysqli_fetch_array($list_country)){
                ...
                if(isset($_POST[$name_check_null])){
                    ...
                    $update_key_lang=mysqli_query($link,"UPDATE `app_my_girl_key_lang` SET `value` = '$value' WHERE `key` = '$key'  AND `lang` = '$key_lang' ");
...
if(isset($_POST['id_edit_key_lang'])){
    ...
    $query_edit_key=mysqli_query($link,"UPDATE `app_my_girl_key_lang` SET `key` = '$key_new' WHERE `key` = '$id_edit_key_lang'");
...
if(isset($_POST['id_delete_key_lang'])){
    ...
    $query_delete=mysqli_query($link,"DELETE FROM `app_my_girl_key_lang` WHERE `key` = '$id_delete_key_lang'");
                ...
                <td>
                    ...
                    if($func=="add"){
                        ...
                        <?php
                            ...
                            $value_is_null="0";
                            $query_get_value=mysqli_query($link,"SELECT `value` FROM `app_my_girl_key_lang` WHERE `key` = '$key' AND `lang` = '$key_lang' LIMIT 1");
...
<form id="form_loc"  method="post" enctype="multipart/form-data" name="edit_key_lang">
    ...
    <div style="display: inline-block;float: left;margin: 2px;">
        ...
        <?php
            $query_all_key=mysqli_query($link,'SELECT DISTINCT `key` FROM app_my_girl_key_lang');
...
if(isset($_GET['del'])){
    $id_del=$_GET['del'];
    $delete_effect=mysqli_query($link,"DELETE FROM `app_my_girl_key_lang` WHERE ((`id` = '$id_del'));");

}


    $list_lang_key=mysqli_query($link,"SELECT * FROM `app_my_girl_key_lang` ORDER BY RAND() LIMIT 50"); 
?>
<table>
<tr>
...
?>
    <tr>
        ...
        <td>
            <a class="buttonPro small orange" href="<?php echo $url.'/app_my_girl_key_lang.php';?>?edit=<?php echo $row['key'];?>&lang=<?php echo $row['lang'];?>"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Sửa giá trị</a>
        </td>
    </tr>
<?php
}
