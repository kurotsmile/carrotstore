...
$sel_rate='-1';
$query_check_rate=mysqli_query($link,"SELECT * FROM `app_my_girl_music_data_$lang_sel` WHERE `device_id` = '$ip_rate' AND `id_chat` = '$id_music' LIMIT 1");
if(isset($_GET['rate'])){
    $sel_rate=$_GET['rate'];
    if(mysqli_num_rows($query_check_rate)){
        $query_update_rate=mysqli_query($link,"UPDATE `app_my_girl_music_data_$lang_sel` SET `value` = '$sel_rate'  WHERE `device_id` = '$ip_rate' AND `device_id` = 'aaaaa' AND `id_chat` = '$id_music' LIMIT 1;");
    }else{
       $query_add_rate=mysqli_query($link,"INSERT INTO `app_my_girl_music_data_$lang_sel` (`device_id`, `value`, `id_chat`) VALUES ('$ip_rate', '$sel_rate', '$id_music');");
    }
...
if(mysqli_num_rows($query_check_rate)){
    $data_rate=mysqli_fetch_array($query_check_rate);
            ...
            <div  style="color: #515151;font-size: 11px;font-weight: normal;margin-top: 20px;float: left;width: 100%;background-color: #ffffffa6;padding: 10px;border-radius: 10px;">
                ...
                <div style="float: left;width: 100%;">
                    <ul>
                        ...
                        $check_music_all_data=mysqli_query($link,"SELECT `value`,`device_id` FROM `app_my_girl_music_data_$lang_music` WHERE `id_chat` = '$id_music' ORDER BY RAND() LIMIT 10");
                        while($row_data=mysqli_fetch_array($check_music_all_data)){
                            $id_row_data_music=$row_data['device_id'];
                            $query_account=mysqli_query($link,"SELECT `name` FROM `app_my_girl_user_$lang_music` WHERE `id_device` = '$id_row_data_music' AND `status`='0' LIMIT 1");
                            if(mysqli_num_rows($query_account)){
                                ...
                                mysqli_free_result($query_account);
                            }
                        }
                        mysqli_free_result($check_music_all_data);
                        ?>
                    </ul>
                </div>
                <?php }?>
